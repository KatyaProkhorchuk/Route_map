...
command_menu.add_command(label='Добавить данные', command = lambda: add_person(db, connection))
...
command_menu.add_command(label="Посмотреть траекторию", command= lambda: get_path(db, connection, map_widget))
command_menu.add_command(label="Посмотреть траекторию всех", command= lambda: get_path_all(db, connection, map_widget))
command_menu.add_command(label="Очистить карту", command= lambda: delete(map_widget))
